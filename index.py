...
def webhook():
    ...
    for x in range(len(myoutputcontexts)):
        if data['queryResult']['action'] == 'DefaultWelcomeIntent.DefaultWelcomeIntent-custom' :
            if myoutputcontexts[x]['name'].rsplit('/', 1)[-1] == 'defaultwelcomeintent-followup' :
                myStorage.setinputSymptom(myoutputcontexts[x]['parameters']['Symptom'])
                myStorage.setcurrentUrl('')
                contextindex = x


    # check for the name of query result and act accordingly
    final_url, reply_text = myStorage.getreply()
    print('-------------------------', file=sys.stdout)
    print(reply_text, file=sys.stdout)

    reply = {}
    reply['fulfillmentText'] = reply_text;
    if (final_url != '') :
        reply['outputContexts'] = [{"name": 'somename', "lifespanCount": 5, "parameters": { 'Homeurl' : '' }}]
        reply['outputContexts'][0]['name'] = "projects/" + str(project_id) + "/agent/sessions/unique/contexts/" + "defaultwelcomeintent-followup"
        reply['outputContexts'][0]['parameters'].update(myoutputcontexts[contextindex]['parameters'])
        reply['outputContexts'][0]['parameters']['Currenturl'] = final_url
